...
const namesIdGreaterNine = pokemon.filter(p => p.id > 99).map(p => p.name)
console.log(namesIdGreaterNine)

const poisonTypePokemon = pokemon.filter(p => p.types.includes("poison"))
console.log(poisonTypePokemon)

const secondTypeFlying = pokemon.filter(p => p.types[1] === "flying").filter(p => p.types[0]).map(p => p.types[0])
